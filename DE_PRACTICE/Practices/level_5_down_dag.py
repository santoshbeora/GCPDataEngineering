...
# DAG Variables
bq_datamart_dataset    = 'dwh_dataset'
...
with DAG(
    ...
    input_sensor = GoogleCloudStorageObjectSensor(
        ...
        object=f'data/signal/staging/{parent_dag}/{execution_date_nodash}/_SUCCESS',
        mode='poke',
        poke_interval=60,
        timeout=60 * 60 * 24 * 7
    )
    ...
    data_mart_sum_total_trips  = BigQueryOperator(
        ...
        sql                         = f""" SELECT DATE(trip_date) as trip_date,
                                      SUM(total_trips) sum_total_trips
                                      FROM `{bq_fact_trips_daily_table_id}`
                                      WHERE DATE(trip_date )= DATE('{execution_date}')
